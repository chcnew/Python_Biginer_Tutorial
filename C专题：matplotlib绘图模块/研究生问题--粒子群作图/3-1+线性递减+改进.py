...
def fit_fun(x):  # 测试函数
    ...
    return res
    # y = np.sum(np.abs(x)) + np.prod(np.abs(x))    # schwefel'sp2.22函数，位置范围在[-10,10]
    # return y
    ...
    def update_dimb(self):  # 定义函数dimb
        ...
        for i in range(self.iter_num):  # 对n个粒子进行循环
            for part in self.Particle_list:  # 粒子的循环
                ...
                self.update_pos(part)  # 更新位置
            # 每次迭代变化
            self.C1 = 1.3 + 1.2 * (i / self.iter_num)  # 第二条曲线的c1的更新策略
            self.C2 = 2 - 1.2 * (i / self.iter_num)  # 第二条曲线的c2的更新策略
    ...
    def update_dimc(self):  # 定义函数dimb
        ...
        for i in range(self.iter_num):  # 对n个粒子进行循环
            for part in self.Particle_list:  # 粒子的循环
                self.update_vel(part)  # 更新速度
                self.update_pos(part)  # 更新位置
                # pos_value = 2 - (i * 2 / self.iter_num) * part.get_pos() + part.get_vel()
            # 每次迭代变化
            self.W = 0.9 - (0.9 - 0.4) * pow(i / self.iter_num, 0.5)  # 第三条曲线的w的更新策略
...
if __name__ == '__main__':  # 当.py文件被直接运行时，if __name__ == '__main__'之下的代码块将被运行，当.py文件以模块形式被导入时，if __name__ == '__main__'之下的代码块不被运行。
    pso1 = PSO(20, 30, 500, 100, 60, 1e-10, C1=2, C2=2, W=1)  # 曲线1的参数设置 pso
    pso2 = PSO(20, 30, 500, 100, 60, 1e-10, C1=1.3, C2=2, W=0.9)  # 曲线2的参数设置 压缩因子pso
    # 第0次传入值
    # w = 0.9
    # c1 = 1.3 + 1.2 * math.cos(0.9 / 200)
    # c2 = 2 - 1.2 * math.cos(0.9 / 200)
    # print(c1, "\n", c2)
    pso3 = PSO(20, 30, 500, 100, 60, 1e-10, C1=1.805, C2=0.695, W=0.9)  # 曲线3的参数设置 对w和c1，c2进行改进
    ...
    fit_var_list3, best_pos3 = pso3.update_dimc()  # 曲线三的最优解和最优值（依据函数dimb更新）
    print("曲线一最优位置:" + str(best_pos1))  # 输出曲线一最优位置
    print("曲线一最优解:" + str(fit_var_list1[-1]))  # 输出曲线一最优位置
    print("曲线二最优位置:" + str(best_pos2))  # 输出曲线二最优位置
    print("曲线二最优解:" + str(fit_var_list2[-1]))  # 输出曲线二最优解
    print("曲线三最优位置:" + str(best_pos3))  # 输出曲线三最优位置
