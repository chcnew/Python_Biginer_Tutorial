...
import mainGUI  # 导入主界面
import registerGUI  # 导入注册界面

rgGUI = None
yxtime = ""  # 有效期
Des_key = "数据库密码"  # Key,需八位
Des_IV = "\x11\2\x2a\3\1\x27\2\0"  # 自定IV向量
Date_Time = "2021-10-26 00:00:00"  # 定义截止日期
...
def checkAuthor():
    ...
    global yxtime
    try:
        with open('register.txt', 'r') as f:  # 无此文件时会报错，直接返回False
            key = f.read()  # 读取所有信息
    except:
        return False
...
def show_reg():
    ...
    global rgGUI
    rgGUI = registerGUI.registerMGUI(version="1.0")  # 创建界面窗口对象（包含窗口及其各个控件）
